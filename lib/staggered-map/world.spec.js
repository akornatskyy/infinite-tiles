...
describe('staggered map / world', () => {
  ...
  it('screen to world rectangle', () => {
    [
      ...
      [-1, -1, -1, -2, 3, 7],
      [0, -193, -1, -8, 3, 7],
    ].forEach(([x, y, tx, ty, tw, th]) => {
      const screen = new Rectangle(x, y, 384, 192);
      const tiles = world.screenToWorldRectangle(screen);
      expect(tiles).toEqual(new Rectangle(tx, ty, tw, th));
    });
  });
});
